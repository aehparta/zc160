...
/******************************************************************************/
int z80w_registers_request(net_session *ns, net_httpd_request *request, net_httpd_response *response, void *userdata)
{
    int err = 0;
    char *data = NULL;
    size_t size = 0;
...
/******************************************************************************/
int z80w_outputs_request(net_session *ns, net_httpd_request *request, net_httpd_response *response, void *userdata)
{
    int err = 0, i;
    char *data = NULL;
    size_t size = 0;
...
int z80w_init()
{
    ...
    z80cpu = z80ex_create(z80w_cpu_rd, NULL,
        z80w_cpu_wr, NULL,
        z80w_cpu_in, NULL,
        z80w_cpu_out, NULL,
        z80w_cpu_intr, NULL);
    ...
    net_httpd_vhost_add_route(httpd_vhost, "/registers\\.json$", z80w_registers_request, NULL);
    net_httpd_vhost_add_route(httpd_vhost, "/outputs\\.json$", z80w_outputs_request, NULL);
...
{
    Z80EX_BYTE err = 0;
    struct mem *m = z80w_find_memory(addr);
